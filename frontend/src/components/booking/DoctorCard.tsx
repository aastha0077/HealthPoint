...
export const DoctorCard: React.FC<DoctorCardProps> = ({ doctor, isSelected, isFavorite, onSelect, onToggleFavorite }) => {
    ...
    return (
        <div
            ...
            className={`relative p-6 rounded-[2rem] border-2 cursor-pointer transition-all duration-300 group overflow-hidden
                ${isSelected
                    ? "border-rose-600 bg-white shadow-2xl shadow-rose-900/10 ring-4 ring-rose-50/50"
                    : "border-slate-50 bg-slate-50/30 hover:bg-white hover:border-slate-100 hover:shadow-xl hover:shadow-slate-200/50"}`}
        >
            {/* Favourite button */}
            <button
                onClick={(e) => { e.stopPropagation(); onToggleFavorite(doctor.id); }}
                className="absolute top-5 right-5 z-10 p-2 rounded-full hover:bg-rose-50 transition-all"
            >
                <Star className={`w-5 h-5 transition-all ${isFavorite ? "fill-rose-500 stroke-rose-500 scale-125" : "fill-none stroke-slate-300 group-hover:stroke-rose-300"}`} />
            </button>

            <div className="flex gap-6 items-center">
                <div className="relative flex-shrink-0 group-hover:scale-105 transition-transform duration-500">
                    <div className="w-20 h-20 rounded-3xl bg-white p-1 shadow-lg border border-slate-50 overflow-hidden">
                         <img
                            src={img}
                    ...
                    {isSelected && (
                        <div className="absolute -bottom-2 -right-2 bg-rose-600 rounded-full border-4 border-white p-1 text-white shadow-md">
                            <BadgeCheck size={16} />
                        </div>
                    )}
                </div>
                <div className="min-w-0 flex-1">
                    ...
                    <h3 className="font-black text-slate-900 text-lg leading-[1.1] tracking-tight">{name}</h3>
