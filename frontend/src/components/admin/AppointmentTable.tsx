            ...
            <div className="bg-white/80 backdrop-blur-xl p-2.5 rounded-xl shadow-sm border border-slate-100 flex flex-wrap gap-2.5 items-center">
                ...
                <div className="flex gap-2.5 items-center">
                    {onExport && (
                        <button 
                            onClick={onExport}
                            className="px-4 py-2 bg-slate-900 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg shadow-slate-200/50 flex items-center gap-2"
                        >
                    ...
                    <table className="w-full text-left whitespace-nowrap">
                        <thead>
                            <tr className="bg-gradient-to-r from-slate-50 to-slate-50/50 border-b border-slate-100">
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400">Patient Details</th>
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400">Doctor & Dept</th>
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400">Schedule</th>
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400">Status</th>
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400">Payment</th>
                                <th className="px-4 py-3 text-[9px] uppercase font-black tracking-widest text-slate-400 text-right">Action</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={a.id} className="hover:bg-slate-50/70 transition-all group">
                                    <td className="px-4 py-2.5">
                                        <div className="flex items-center gap-2.5">
                                            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-slate-100 to-slate-50 flex items-center justify-center text-slate-500 font-black text-[10px] border border-slate-100">
                                                {(a.patient?.firstName || "P")[0]}
                                            </div>
                                            <div>
                                                <p className="font-bold text-slate-900 text-xs">{a.patient?.firstName} {a.patient?.lastName}</p>
                                                <div className="flex items-center gap-1.5 mt-0.5">
                                                    <span className="text-[7px] font-black text-rose-500 uppercase tracking-widest">{a.patient?.gender}</span>
                                                    <span className="w-0.5 h-0.5 rounded-full bg-slate-200" />
                                                    <span className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">{a.patient?.age} YRS</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2.5">
                                        <p className="font-bold text-slate-700 text-xs">Dr. {a.doctor?.user?.firstName || a.doctor?.firstName} {a.doctor?.user?.lastName || a.doctor?.lastName}</p>
                                        <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{a.doctor?.department?.name || 'Registry'}</p>
                                    </td>
                                    <td className="px-4 py-2.5">
                                        ...
                                        <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest pl-3.5 mt-0.5">
                                            {new Date(a.dateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                        </div>
                                    </td>
                                    <td className="px-4 py-2.5">
                                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-widest border ${a.status === 'BOOKED' ? 'bg-blue-50 text-blue-600 border-blue-100' :
                                    ...
                                    <td className="px-4 py-2.5 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            })} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all border border-transparent hover:border-rose-100" title="Delete Appointment">
                                                <Trash2 size={12} />
                                            </button>
                                        </div>
                                    </td>
